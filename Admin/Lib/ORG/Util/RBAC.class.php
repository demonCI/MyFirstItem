...
class RBAC {
    ...
    static public function authenticate($map,$model='')
    {
        ...
        //使用给定的Map进行认证
        return M($model)->where($map)->select();
    }
    ...
    static public function getAccessList($authId)
    {
        ...
        $sql    =   "select node.id,node.name from ".
                    ...
                    $table['node']." as node ".
                    "where user.user_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=1";
        $apps =   $db->query($sql);
        $access =  array();
        foreach($apps as $key=>$app) {
            ...
            $sql    =   "select node.id,node.name from ".
                    ...
                    $table['access']." as access ,".
                    $table['node']." as node ".
                    "where user.user_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=2 and node.pid={$appId}";
            //Log::write($sql,LOG::SQL);
	    ...
	    //Log::write(print_r($modules,true),LOG::INFO);
            ...
            foreach($modules as $key=>$module) {
                ...
                $sql    =   "select node.id,node.name from ".
                    ...
                    $table['access']." as access ,".
                    $table['node']." as node ".
                    "where user.user_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=3 and node.pid={$moduleId}";
                    $rs =   $db->query($sql);
                    foreach ($rs as $a){
