...
class OrderAction extends CommonAction {
    ...
    public function _before_index() {
        $this->assign('brandList', D('brand')->select()); //品牌名称
        $this->assign('shopList', D('shop')->select()); //店铺名称
        $this->assign('projectList', D('project')->where('type=2')->select()); //工程名称
    }
    ...
    public function index() {
        ...
        $map = $this->_search();
        $map['order_type'] = 1;
        if (!empty($_REQUEST['pbegin']) && !empty($_REQUEST['pend'])) {
            $map['ptime'] = array(array('egt', $_REQUEST ['pbegin']), array('elt', $_REQUEST ['pend']));
        }
        $_SESSION['map'] = $map;
    ...
    public function Order_refuse() {
        $order_reason=$_REQUEST['order_reason'];
       
        $this->assign('order_reason',$order_reason);
        $this->display();
        
    }

//    function _before_index() {
//        $this->assign("inter", getFieldArray("Express"));
...
//    }
    ...
    public function excel() {
        ...
        $title = array('店铺名称', '工程名称', '申请时间', '施工人员名称', '施工人员电话', '订单编号',  '订单状态');
        $rs = array(); //数据库获取数据
...
//                    ->setCellValue('H' . $col, getCarflago($v['overdraft']))
    ...
    public function pdf() {
        $map = $_SESSION['map'];
        $list = D('order')->where($map)->field('shop_id,project_id,ptime,project_phone,status')->select();
        Log::write(D('order')->getLastSql(), LOG::SQL);
        $result = array();
        if (!empty($list)) {
            $rs = array();
            foreach ($list as $key => $value) {
                ...
                $rs['ptime'] = $value['ptime'];
                $rs['reason'] = $value['reason'];
                $rs['project_name'] = $value['project_name'];
                $rs['project_phone'] = $value['project_phone'];
                $rs['status'] = getStatus($value['status']);
                $result[] = $rs;
            }
        }
        $title = array('店铺名称', '工程名称', '申请时间', '施工人员名称', '施工人员电话', '订单状态');
        $this->export_pdf($title, $result);
        $this->success('操作成功');
    }
